...
try:
    ...
    print("➡️  Please check MySQL server status and credentials.")
...
def api_diagnose():
    ...
    if best_match and highest_score > 60:
        ...
        with db_cursor() as cursor:
            cursor.execute(
                "INSERT INTO chat_history (symptom, diagnosis, created_at) VALUES (%s, %s, NOW())",
                (symptoms, diagnosis)
            )
...
def get_chat_history():
    try:
        with db_cursor() as cursor:
            cursor.execute(
                "SELECT symptom, diagnosis, created_at FROM chat_history ORDER BY created_at DESC LIMIT 10"
            )
